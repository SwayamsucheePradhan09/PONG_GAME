#include <SFML/Graphics.hpp> // Includes the necessary SFML graphics classes
using namespace sf; // Allows us to drop the "sf::" prefix for SFML objects

class Ball {
private:
    CircleShape ballShape; // The visual representation of the ball
...
public:
    ...
    CircleShape returnShape(); // Returns the ball's shape so it can be drawn
    
    void upadteBall(Time &dt); // Moves the ball based on elapsed time (Delta Time)
    ...
    void ballTochesBat(FloatRect batBound, bool &incScore); // Handles bouncing off the player's bat
};